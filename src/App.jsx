...
const CONFIG = {
  // 2. IZMAIŅA: Notion atslēgas paslēptas
  ...
  // 3. IZMAIŅA (SVARĪGI): Nomainīts uz "tuneli", lai novērstu "Failed to fetch"
  baseUrl: "/api/notion"
};
...
const NotionService = {
  ...
  async getMealPlan() {
    try {
      ...
      // We keep this map to handle legacy Latvian data in Notion if it exists
      const DAY_MAP = { 
      ...
      return data.results.map(page => {
        ...
        return {
          ...
          day: DAY_MAP[rawDay] || "Monday", // Always converts to English
...
export default function App() {
  ...
  // App State (Language state removed!)
  ...
  const saveDefaultPortions = () => {
    ...
    alert("Default serving size saved!");
  };
  ...
  const loadNotionData = async () => {
    ...
    try {
      ...
      if (meals.length > 0) {
        ...
        setErrorMsg(`Notion Error: ${err.message}`); 
    } 
  ...
  // 3. ADD ITEM (Departments now in English)
  const handleAddCustomItem = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg("Error saving to Firebase.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // --- SMART FRIDGE LOGIC (Simplified for English) ---
 ...
 const groupedItems = useMemo(() => {
    ...
    // Note: If you have old LV items in Firebase, they will appear at the bottom
    const sortedResult = {};
  ...
  return (
    <div className={`relative max-w-md mx-auto min-h-screen bg-[#F8FAFC] font-sans text-slate-900 flex flex-col pb-32 transition-all`}>
      ...
      {/* --- SETTINGS SCREEN (English & Compact) --- */}
      {isSettingsOpen && (
        <div className="fixed top-24 left-0 right-0 z-[150] px-6 max-w-md mx-auto animate-in slide-in-from-top-4 pointer-events-none">
          <div className="bg-white border border-slate-100 rounded-[3rem] p-6 shadow-2xl pointer-events-auto relative">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100/50">
                  <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">User</p>
                          <h3 className="text-xs font-black italic text-slate-900 leading-tight truncate max-w-[120px]">{user.email}</h3>
                      </div>
                  </div>
                  <button onClick={handleLogout} className="p-3 bg-white text-red-400 hover:text-red-500 rounded-xl shadow-sm border border-slate-100 active:scale-90 transition-all" title="Sign Out">
                      <LogOut size={18}/>
                  </button>
              </div>
              ...
              {/* Theme Selector */}
              <div>
                 <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest mb-2 px-2">Color Theme</p>
                 <div className="flex justify-between gap-2">
                    {Object.keys(THEMES).map(k => (
                       <button 
                          key={k} 
                          onClick={() => setCurrentTheme(k)} 
                          className={`flex-1 h-10 rounded-xl border flex items-center justify-center transition-all ${currentTheme === k ? `${THEMES[k].bgLight} ${THEMES[k].border} shadow-sm ring-1 ring-offset-1 ring-slate-100` : 'bg-white border-slate-100 hover:bg-slate-50'}`}
                          title={THEMES[k].name}
                       >
              ...
              <div className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between">
                 <div>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Default Serving Size</p>
                    <p className="text-[9px] text-slate-300 font-medium">Family size: {portions}</p>
                 </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
           <div className="bg-white rounded-[3rem] p-8 w-full max-w-sm shadow-2xl relative animate-in slide-in-from-bottom-4 pointer-events-auto">
              <div className="flex justify-between items-center mb-6">
                 <h3 className="text-xl font-black italic uppercase tracking-tighter">Add Item</h3>
             ...
             <form onSubmit={handleAddCustomItem} className="space-y-4">
                 ...
                 <div className="flex gap-4">
                    <div className="space-y-1 flex-1">
                       ...
                       <input type="number" min="0.1" step="0.1" required value={newItem.Amount} onChange={e => setNewItem({...newItem, Amount: parseFloat(e.target.value)})} className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-100 font-black italic focus:outline-none focus:border-orange-500" />
                    </div>
                    <div className="space-y-1 flex-1">
                       <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-2">Unit</label>
                       <select value={newItem.Unit} onChange={e => setNewItem({...newItem, Unit: e.target.value})} className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-100 font-bold italic focus:outline-none focus:border-orange-500 appearance-none">
                          <option value="pcs">pcs</option><option value="kg">kg</option><option value="g">g</option><option value="l">l</option><option value="ml">ml</option><option value="pack">pack</option>
                       </select>
                    </div>
                 </div>
                 <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-2">Department</label>
                    <select value={newItem.Department} onChange={e => setNewItem({...newItem, Department: e.target.value})} className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-100 font-bold italic focus:outline-none focus:border-orange-500 appearance-none">
                       <option value="Produce">Produce</option><option value="Dairy">Dairy</option><option value="Meat & Fish">Meat & Fish</option><option value="Bakery">Bakery</option><option value="Pantry">Pantry</option><option value="Frozen">Frozen</option><option value="Household">Household</option><option value="Other">Other</option>
                    </select>
                 </div>
      ...
      <header className="px-6 pt-8 pb-4 flex justify-between items-start z-[110]">
        <div>
          <h1 className="text-3xl font-black italic uppercase tracking-tighter leading-none text-slate-900">Kitchen<br/><span className={theme.text}>Hub</span></h1>
          <div className="flex items-center gap-1.5 mt-2"><div className={`w-1.5 h-1.5 rounded-full bg-green-500`}></div><span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 italic">Synced</span></div>
        </div>
        
        <div className="flex flex-col items-end gap-3">
           <div className="flex gap-3">
              <button onClick={()=>setIsSettingsOpen(!isSettingsOpen)} className="w-10 h-10 bg-white rounded-full shadow-sm flex items-center justify-center text-slate-600 active:scale-90 transition-all border border-slate-100">
                 <Settings size={18} />
              </button>
              <button onClick={loadNotionData} className="w-10 h-10 bg-white rounded-full shadow-sm flex items-center justify-center text-slate-600 active:scale-90 transition-all border border-slate-100">
      ...
      <main className="flex-1 p-6 pt-0 space-y-6 overflow-y-auto">
        {selectedRecipe ? (
          ...
          <div className="space-y-6">
            ...
            {/* --- COMPACT SERVING SIZE (MOVED TO TOP) --- */}
            ...
            <div className="flex items-center justify-between bg-slate-900 p-4 px-6 rounded-[2rem] shadow-lg mb-6 active:scale-[0.99] transition-transform">
               <button onClick={() => setSelectedDayIndex(i => (i - 1 + 7) % 7)} className="p-2 text-slate-500 hover:text-white transition-colors">
                  <ChevronLeft size={24} />
               </button>
               <span className={`text-xl font-black italic uppercase tracking-widest ${theme.text}`}>
                  {DAYS[selectedDayIndex]}
               </span>
               <button onClick={() => setSelectedDayIndex(i => (i + 1) % 7)} className="p-2 text-slate-500 hover:text-white transition-colors">
                  <ChevronRight size={24} />
               </button>
            </div>

            <div className="relative">
              <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="w-full bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm flex items-center justify-between group active:scale-[0.98]"><div className="flex items-center gap-4"><div className={`p-3 bg-slate-900 text-white rounded-2xl`}><List size={18} /></div><div className="text-left"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">MENU</p><p className="text-sm font-black uppercase italic tracking-tight">{selectedMenu || "Select Menu"}</p></div></div><ChevronDown size={20} className={`text-slate-300 transition-transform ${isMenuOpen ? 'rotate-180' : ''}`} /></button>
            ...
            <div className="space-y-4">{currentDayPlan.length > 0 ? currentDayPlan.map(m => (<div key={m.id} onClick={() => setSelectedRecipe(m)} className="flex items-center gap-5 p-6 rounded-[2.5rem] border border-slate-100 shadow-sm bg-white cursor-pointer active:scale-[0.98] transition-all"><div className={`w-16 h-16 rounded-3xl flex items-center justify-center ${m.type.includes('Breakfast') ? 'bg-amber-50 text-amber-500' : m.type.includes('Lunch') ? 'bg-blue-50 text-blue-500' : 'bg-indigo-50 text-indigo-500'}`}>{m.type.includes('Breakfast') ? <Coffee size={28}/> : m.type.includes('Lunch') ? <Sun size={28}/> : <Moon size={28}/>}</div><div className="flex-1 min-w-0 text-left"><p className="text-[9px] font-black uppercase tracking-wider text-slate-400 mb-0.5">{m.type}</p><h3 className="font-black truncate text-base uppercase italic tracking-tight text-slate-800">{m.name}</h3></div><div className="p-2 bg-slate-50 rounded-xl"><ChevronRight size={16} className="text-slate-300"/></div></div>)) : <div className="p-16 text-center opacity-20 italic font-black text-xs tracking-widest border-2 border-dashed border-slate-200 rounded-[3rem]">No Plan</div>}</div>
          </div>
        ) : (
            ...
            {Object.keys(groupedItems).length > 0 ? Object.keys(groupedItems).map(d => (
                <div key={d} className="space-y-4">
                    ...
                    <div className="space-y-2">{groupedItems[d].map(i => (<div key={i.id} onClick={() => setCheckedItems(p => ({ ...p, [i.id]: !p[i.id] }))} className={`p-5 px-7 bg-white border rounded-[1.8rem] flex justify-between items-center shadow-sm cursor-pointer transition-all ${checkedItems[i.id] ? `${theme.border} ${theme.bgLight} opacity-60` : 'border-slate-100'}`}> <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className={`font-black text-[14px] uppercase italic ${checkedItems[i.id] ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{i.Item || "Unknown Item"}</h3>
                                </div>
