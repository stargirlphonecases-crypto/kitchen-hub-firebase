...
import { 
  ...
  Refrigerator // <-- JAUNA IKONA
} from 'lucide-react';

// --- FIREBASE IMPORTS ---
import { initializeApp } from "firebase/app";
import { 
  getFirestore, collection, onSnapshot, doc, updateDoc, setDoc, addDoc, query, getDoc, deleteDoc
...
export default function App() {
  ...
  // 'inventory' tab tagad ir 'cart', lai nemulsinātu, bet iekšēji atstāsim, lai mazāk izmaiņu
  // 'fridge' ir jaunais ledusskapja skats
  ...
  const handleAddCustomItem = async (e) => {
    ...
    try {
      ...
      // LOGIC CHANGE: Ja esam "fridge" tabā, mēs PIESKAITĀM krājumus. Ja esam "cart" (inventory) tabā, mēs ATŅEMAM (deficīts).
    ...
    try {
      const batchPromises = idsToUpdate.map(async (id) => {
        ...
        if (item) {
           ...
           // Ja mēs "finish shopping", tas nozīmē, ka deficīts ir nosegts. Stock kļūst 0 (vai pozitīvs, ja nopirkām vairāk, bet pagaidām vienkārši nullējam deficītu).
           // Pareizāk būtu: ja inStock bija -2, un mēs nopirkām, tad tagad ir 0? Vai mēs nopirkām 2?
           // Šobrīd "Finish Shopping" vienkārši nodzēš vajadzību (reset to 0), kas nozīmē "Mums ir tieši tik, cik vajag šim brīdim".
           // Nākotnē to var uzlabot.
  ...
  // Jauna funkcija: Manuāli mainīt daudzumu ledusskapī
  const updateStock = async (id, delta) => {
    ...
    if (item) {
        ...
        // Ja stock nokrīt uz 0 un tas nav receptē (nav Notion saites), vai dzēst? Pagaidām atstājam 0.
        await updateDoc(itemRef, { inStock: newStock });
    }
  };
 ...
 const groupedItems = useMemo(() => {
    ...
    allItemNames.forEach(normName => {
        ...
        // MATEMĀTIKA: Ja inStock ir pozitīvs (piem. 5 olas), un vajag 3, tad finalNeed ir -2 (negatīvs = nav jāpērk).
        // Ja vajag 10, ir 5, tad finalNeed ir 5.
        const finalNeed = recipeNeed - inStock;
        
        if (finalNeed <= 0.1) return; // Rādām grozā tikai to, kas trūkst (ar nelielu buferi)
  ...
  // Filtrējam preces, kas ir "Ledusskapī" (Stock > 0)
    ...
    <div className={`relative max-w-md mx-auto min-h-screen bg-[#F8FAFC] font-sans text-slate-900 flex flex-col pb-32 transition-all`}>
      ...
      <main className="flex-1 p-6 pt-0 space-y-6 overflow-y-auto">
        {selectedRecipe ? (
          ...
          // --- JAUNAIS LEDUSSKAPJA SKATS (NEW FRIDGE VIEW) ---
                ...
                {fridgeItems.length > 0 ? fridgeItems.map(i => (
                    <div key={i.id} className="p-5 px-7 bg-white border border-slate-100 rounded-[1.8rem] flex justify-between items-center shadow-sm">
                       <div className="flex items-center gap-4">
                           ...
                           <div><h3 className="font-black text-[14px] uppercase italic text-slate-700">{i.Item}</h3><p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">{i.Department || "Other"}</p></div>
                       </div>
                       <div className="flex items-center gap-3">
                           ...
                           <div className="text-center min-w-[40px]"><span className="text-lg font-black italic">{i.inStock}</span><span className="text-[9px] block uppercase font-bold text-slate-300">{i.Unit}</span></div>
      ...
      <div className="fixed bottom-6 left-0 right-0 z-[100] px-6 max-w-md mx-auto pointer-events-none">
        <nav className="bg-slate-900 border border-white/5 p-2 rounded-[2.5rem] flex items-center justify-around shadow-2xl pointer-events-auto">
          {/* NAVIGATION BAR - NOW WITH 3 ITEMS */}
