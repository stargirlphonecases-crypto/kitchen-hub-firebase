...
export default function App() {
  ...
  const masterItemList = useMemo(() => {
    ...
    // 1. Pievienojam produktus no Notion (Standarts)
    recipeIngredients.forEach(ri => {
        const name = (ri.Item || "").trim();
        if (name) {
            itemsMap.set(name.toLowerCase(), {
                name: name, // Saglabājam oriģinālo rakstību
                unit: ri.Unit || "pcs",
                dept: "Other" // Notion parasti nav nodaļu, to ņemsim no vēstures
            });
        }
    });

    // 2. Pievienojam produktus no Firebase (Vēsture un Nodaļas)
    // Ja produkts jau ir, mēs atjaunojam Nodaļu (jo Firebase nodaļa ir precīzāka)
    inventory.forEach(inv => {
        ...
        if (name) {
            ...
            if (existing) {
                // Atjaunojam info ar lietotāja vēsturisko izvēli (Department)
                itemsMap.set(key, { ...existing, dept: inv.Department || existing.dept });
            } else {
                // Jauns produkts (ko lietotājs reiz ievadījis, bet nav receptēs)
                itemsMap.set(key, {
                    name: name,
                    unit: inv.Unit || "pcs",
                    dept: inv.Department || "Other"
                });
            }
        }
    });
    ...
    try {
      const batchPromises = idsToUpdate.map(async (id) => {
        ...
        if (isVirtual) {
            const invCol = collection(db, "households", householdId, "inventory");
            await addDoc(invCol, { 
                Item: item.Item, Unit: item.Unit, Department: item.Department, inStock: newStock 
            });
        } else {
 ...
 const groupedItems = useMemo(() => {
  ...
  return (
    <div className={`relative max-w-md mx-auto min-h-screen bg-[#F8FAFC] font-sans text-slate-900 flex flex-col pb-32 transition-all`}>
      ...
      {/* --- ADD ITEM MODAL WITH AUTOCOMPLETE --- */}
      {isAddModalOpen && (
                        ...
                        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-xl border border-slate-100 max-h-48 overflow-y-auto z-50">
                            {suggestions.map((s, idx) => (
                                <div 
                                    ...
                                    className="p-3 px-4 hover:bg-slate-50 cursor-pointer border-b border-slate-50 last:border-0 flex justify-between items-center"
                                >
