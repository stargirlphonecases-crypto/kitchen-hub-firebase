    ...
    try {
      const batchPromises = idsToUpdate.map(async (id) => {
        ...
        if (item) {
           ...
           // 1. Aprēķinām receptes vajadzību šai precei
           const normName = (item.Item || "").trim().toLowerCase();
           ...
           // 2. Jaunais atlikums = "Mums tagad ir tik, cik vajag receptei"
           // Ja bija 2, vajadzēja 10, nopirkām 8 -> rezultāts 10.
           let newStock = 0;
           
           if (totalRecipeNeed > 0) {
               newStock = totalRecipeNeed;
           } else {
               // Ja tā bija manuāla groza prece (Stock < 0), tad tagad tā ir nopirkta (Stock = 0)
               newStock = 0;
           }
           
           // Drošība: ja mums jau bija vairāk (Stock > Need), tad nesamazinām
           if ((item.inStock || 0) > newStock) newStock = item.inStock;
