...
import { 
  getFirestore, collection, onSnapshot, doc, updateDoc, setDoc, addDoc, query, getDoc
...
export default function App() {
  ...
  const [householdId, setHouseholdId] = useState(null); // Šis ir atslēga uz kopējo ledusskapi
  ...
  const [joinCodeInput, setJoinCodeInput] = useState(""); // Lauks, kur ievadīt cita ID
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      setUser(currentUser);
      if (currentUser) {
        // Tiklīdz lietotājs ielogojas, mēs meklējam viņa "householdId"
        try {
          const userDocRef = doc(db, "users", currentUser.uid);
          const userDocSnap = await getDoc(userDocRef);

          if (userDocSnap.exists()) {
            // Ja lietotājam jau ir profils, paņemam viņa mājsaimniecības ID
            setHouseholdId(userDocSnap.data().householdId);
          } else {
            // Jauns lietotājs: izveidojam profilu un iestatām, ka viņš ir savas mājas saimnieks
            await setDoc(userDocRef, { 
              email: currentUser.email,
              householdId: currentUser.uid // Pēc noklusējuma Household ID = User ID
            });
  ...
  // 2. INVENTORY LISTENER (MULTI-USER SUPPORT)
  useEffect(() => {
    // Mēs klausāmies datus TIKAI tad, ja ir zināms, KURĀ mājsaimniecībā skatīties
    if (!user || !householdId) return; 

    // Jaunā struktūra: households / {ID} / inventory
    ...
    const unsubscribe = onSnapshot(inventoryRef, (querySnapshot) => {
      ...
      setErrorMsg("Connection to household database failed.");
    });

    return () => unsubscribe();
  }, [user, householdId]); // Pārlādējas, ja nomainās mājsaimniecība

  // --- HOUSEHOLD JOINING LOGIC ---
  const handleJoinHousehold = async () => {
    if (!joinCodeInput.trim()) return;
    setLoading(true);
    try {
      // 1. Pārbaudām, vai tāda mājsaimniecība eksistē (vai vismaz vai ir tāds lietotājs, kura ID šis ir)
      // Mēs vienkārši atjaunojam lietotāja profilu ar jauno ID
      const userDocRef = doc(db, "users", user.uid);
      await updateDoc(userDocRef, {
        householdId: joinCodeInput.trim()
      });
      
      // Atjaunojam lokālo stāvokli, lai Inventory Listener pārslēgtos
      setHouseholdId(joinCodeInput.trim());
      alert("Joined new household successfully!");
      setJoinCodeInput("");
    } catch (e) {
      alert("Error joining household: " + e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const copyToClipboard = () => {
    ...
    alert("Your Household Code copied!");
  };

  // ... (Pārējās funkcijas paliek nemainīgas, bet izmanto householdId) ...
  ...
  const handleAddCustomItem = async (e) => {
    ...
    setIsAddModalOpen(false); setLoading(true);
    try {
      // SVARĪGI: Rakstām konkrētās mājsaimniecības ledusskapī
      ...
      if (existing) {
        ...
        await updateDoc(itemRef, { inStock: (existing.inStock || 0) - Math.abs(newItem.Amount) });
      } else {
        const invCol = collection(db, "households", householdId, "inventory");
        await addDoc(invCol, { ...newItem, inStock: -Math.abs(newItem.Amount) });
      }
 ...
 const groupedItems = useMemo(() => {
  ...
  return (
    <div className={`relative max-w-md mx-auto min-h-screen bg-[#F8FAFC] font-sans text-slate-900 flex flex-col pb-32 transition-all`}>
      {/* --- SETTINGS SCREEN (UPDATED FOR HOUSEHOLD SHARING) --- */}
      {isSettingsOpen && (
              ...
              <div className="space-y-3">
                 ...
                 <div className="bg-slate-900 text-white p-5 rounded-2xl shadow-lg">
                    ...
                    <div className="flex items-center gap-2">
                       <code className="flex-1 bg-white/10 p-2 rounded-lg text-xs font-mono truncate">{user.uid}</code>
                       <button onClick={copyToClipboard} className="p-2 bg-white/20 rounded-lg hover:bg-white/30 active:scale-90"><Copy size={14}/></button>
                    </div>
                    <p className="text-[9px] text-slate-500 mt-2 leading-tight">Share this code with family members to let them join your fridge.</p>
                 </div>

                 <div className="bg-white p-4 rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Join a Household</p>
                    <div className="flex gap-2">
                       <input 
                          ...
                          className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold focus:outline-none focus:border-slate-400"
                       />
                       <button onClick={handleJoinHousehold} className={`px-4 rounded-xl text-white font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 ${theme.primary}`}>Join</button>
                    </div>
                 </div>
              </div>
              ...
              <div>
                 ...
                 <div className="flex justify-between gap-2 mb-3">
                    {Object.keys(THEMES).map(k => (
                       <button key={k} onClick={() => setCurrentTheme(k)} className={`flex-1 h-10 rounded-xl border flex items-center justify-center transition-all ${currentTheme === k ? `${THEMES[k].bgLight} ${THEMES[k].border} shadow-sm ring-1 ring-offset-1 ring-slate-100` : 'bg-white border-slate-100 hover:bg-slate-50'}`}><div className={`w-3.5 h-3.5 rounded-full ${THEMES[k].primary}`}></div></button>
                    ))}
                 </div>
                 <div className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between">
                    <div><p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Family Size</p><p className="text-[9px] text-slate-300 font-medium">{portions} People</p></div>
                    <div className="flex items-center gap-2">
      ...
      {/* ... (ADD MODAL, HEADER, MAIN CONTENT - VISS PĀRĒJAIS PALIEK KĀ IR) ... */}
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
           <div className="bg-white rounded-[3rem] p-8 w-full max-w-sm shadow-2xl relative animate-in slide-in-from-bottom-4 pointer-events-auto">
              <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-black italic uppercase tracking-tighter">Add Item</h3><button onClick={() => setIsAddModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-800"><X size={24}/></button></div>
             <form onSubmit={handleAddCustomItem} className="space-y-4">
                 <div className="space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-2">Item Name</label><input type="text" required value={newItem.Item} onChange={e => setNewItem({...newItem, Item: e.target.value})} className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-100 font-bold italic focus:outline-none focus:border-orange-500" /></div>
                 ...
                 <button type="submit" className={`w-full mt-4 p-5 rounded-2xl text-white font-black uppercase italic tracking-widest shadow-lg flex justify-center items-center gap-2 active:scale-95 transition-all ${theme.primary}`}><Plus size={20}/> Add Item</button>
              </form>
           </div>
        </div>
      )}
