...
const CONFIG = {
  ...
  // Mēs atstājam Netlify tuneli, jo ar kešatmiņu tas būs droši un "profesionāli"
  baseUrl: "/api/notion" 
};

// --- FEEDBACK CONFIG ---
const FEEDBACK_EMAIL = "info@virtuveshub.lv"; 
const CACHE_DURATION_MS = 15 * 60 * 1000; // 15 minūtes kešatmiņa
...
export default function App() {
  ...
  const [isSettingsOpen, setIsSettingsOpen] = useState(false); // App Settings (Gear)
  const [isProfileOpen, setIsProfileOpen] = useState(false);   // User Profile (User Icon)
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        try {
          ...
          loadNotionData(false); // Sākotnējā ielāde, izmanto kešatmiņu ja ir
        } catch (e) {
 ...
 const groupedItems = useMemo(() => {
    ...
    allItemNames.forEach(normName => {
        ...
        // LABOJUMS 1: Ja izvēlētā ēdienkarte šo produktu neprasa, mēs to uzreiz ignorējam!
        if (recipeNeed <= 0) return;

        const fireItem = inventory.find(i => (i.Item || "").trim().toLowerCase() === normName);
        const recipeInfo = recipeIngredients.find(r => (r.Item || "").trim().toLowerCase() === normName);
        
        // LABOJUMS 2: Ja noliktavā ir mīnusi (no iepriekšējiem testiem), mēs tos ignorējam un uztveram kā 0.
