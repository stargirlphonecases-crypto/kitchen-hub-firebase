...
const NotionService = {
  async request(endpoint, method = 'POST', body = null) {
    // FIX for URL parsing error in sandbox
...
export default function App() {
 ...
 const groupedItems = useMemo(() => {
    ...
    allItemNames.forEach(normName => {
        ...
        // SPOKU ĶĒRĀJS 1: Ja izvēlētā recepte šo produktu neprasa, vispār ignorējam!
        if (recipeNeed <= 0) return;

        const fireItem = inventory.find(i => (i.Item || "").trim().toLowerCase() === normName);
        const recipeInfo = recipeIngredients.find(r => (r.Item || "").trim().toLowerCase() === normName);
        
        // SPOKU ĶĒRĀJS 2: Ja Firebase ledusskapī ir mīnusi (parādi), uztveram tos kā 0
