...
const CONFIG = {
  ...
  ingredientsDbId: process.env.REACT_APP_NOTION_INGREDIENTS_DB_ID,
  mealPlansDbId: process.env.REACT_APP_NOTION_MEALPLANS_DB_ID,
...
export default function App() {
  ...
  const handleEmptyFridge = async () => {
      ...
      try {
          ...
          const deletePromises = snapshot.docs.map(d => deleteDoc(d.ref));
          await Promise.all(deletePromises);
          alert("Fridge emptied successfully! 🧹");
          setIsSettingsOpen(false);
 ...
 const groupedItems = useMemo(() => {
    ...
    allItemNames.forEach(normName => {
        ...
        // SPOKU ĶĒRĀJS 1: Ignorē produktus, kurus neprasa šī ēdienkarte
        if (recipeNeed <= 0) return;

        const fireItem = inventory.find(i => (i.Item || "").trim().toLowerCase() === normName);
        const recipeInfo = recipeIngredients.find(r => (r.Item || "").trim().toLowerCase() === normName);
        
        // SPOKU ĶĒRĀJS 2: Ignorē ledusskapja mīnusus
        const inStock = fireItem ? Math.max(0, fireItem.inStock || 0) : 0;
                 ...
                 <div className="space-y-2">
                     <button onClick={handleResetWeek} className="w-full p-4 bg-white border border-slate-200 rounded-2xl flex items-center justify-between group active:scale-95 transition-all hover:bg-slate-50 hover:border-slate-300">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left"><h3 className="text-sm font-black uppercase italic text-slate-700">Start New Week</h3><p className="text-[9px] font-bold text-slate-300">Clear all 'Cooked' statuses</p></div>
                        </div>
                        <ChevronRight size={16} className="text-slate-300"/>
                     </button>
                     
                     {/* POGA IZTĪRĪT LEDUSSKAPI */}
                     <button onClick={handleEmptyFridge} className="w-full p-4 bg-white border border-red-100 rounded-2xl flex items-center justify-between group active:scale-95 transition-all hover:bg-red-50 hover:border-red-200">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left"><h3 className="text-sm font-black uppercase italic text-red-600">Empty Fridge</h3><p className="text-[9px] font-bold text-red-400">Delete all inventory data</p></div>
                        </div>
