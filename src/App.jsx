...
import { 
  ...
  Refrigerator, Search, Flame, CalendarX, MessageSquare, Users, MessageCircle
} from 'lucide-react';
...
export default function App() {
  ...
  // LATVISKAIS NOKLUSĒJUMS
  const [newItem, setNewItem] = useState({ Item: "", Amount: 1, Unit: "gab", Department: "Cits" });
  ...
  const masterItemList = useMemo(() => {
    ...
    recipeIngredients.forEach(ri => {
        ...
        if (name) itemsMap.set(name.toLowerCase(), { name: name, unit: ri.Unit || "gab", dept: ri.Department || "Cits" });
    });
    inventory.forEach(inv => {
        ...
        if (name) {
            ...
            else itemsMap.set(key, { name: name, unit: inv.Unit || "gab", dept: inv.Department || "Cits" });
        }
    });
  ...
  const handleAddCustomItem = async (e) => {
    ...
    try {
      ...
      setNewItem({ Item: "", Amount: 1, Unit: "gab", Department: "Cits" });
      ...
      const batchPromises = idsToUpdate.map(async (id) => {
        ...
        if (isVirtual) {
            ...
            item = {
                ...
                Unit: recipeInfo ? recipeInfo.Unit : "gab",
                Department: recipeInfo?.Department || "Cits",
                inStock: 0
            };
        } else {
    ...
    try {
      ...
      const batchPromises = ingredientsToDeduct.map(async (ri) => {
          ...
          if (fireItem) {
              ...
              const invCol = collection(db, "households", householdId, "inventory");
              await addDoc(invCol, { Item: ri.Item, Unit: ri.Unit || "gab", Department: ri.Department || "Cits", inStock: -amountToDeduct });
          }
      });
 ...
 const groupedItems = useMemo(() => {
    ...
    allItemNames.forEach(normName => {
        ...
        const department = fireItem?.Department || recipeInfo?.Department || "Cits";
        ...
        result[department].push({
            ...
            Unit: fireItem?.Unit || recipeInfo?.Unit || "gab"
        });
    });
    // LATVISKIE DEPARTAMENTI
    const DEPARTMENT_ORDER = ["Dārzeņi un augļi", "Gaļa un zivis", "Piena produkti", "Maize", "Bakaleja", "Saldētie produkti", "Saimniecības preces", "Cits"];
    const sortedResult = {};
  ...
  const fridgeItems = useMemo(() => {
      return inventory.filter(i => (i.inStock || 0) > 0).sort((a,b) => (a.Department || "Cits").localeCompare(b.Department || "Cits"));
  }, [inventory]);
        ...
        <div className="fixed top-24 left-0 right-0 z-[150] px-6 max-w-md mx-auto animate-in slide-in-from-top-4 pointer-events-none">
          <div className="bg-white border border-slate-100 rounded-[3rem] p-6 shadow-2xl pointer-events-auto relative">
            <div className="flex justify-between items-center mb-4 px-2">
              ...
              <button onClick={() => setIsProfileOpen(false)} className="bg-slate-50 p-2 rounded-full active:scale-90 transition-transform hover:bg-slate-100"><X size={18} className="text-slate-400"/></button>
            </div>
              ...
              <div>
                 ...
                 <button onClick={handleResetWeek} className="w-full p-4 bg-white border border-slate-200 rounded-2xl flex items-center justify-between group active:scale-95 transition-all hover:bg-red-50 hover:border-red-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-left"><h3 className="text-sm font-black uppercase italic text-slate-700 group-hover:text-red-600">Start New Week</h3><p className="text-[9px] font-bold text-slate-300 group-hover:text-red-300">Clear all 'Cooked' statuses</p></div>
                    </div>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
           <div className="bg-white rounded-[3rem] p-8 w-full max-w-sm shadow-2xl relative animate-in slide-in-from-bottom-4 pointer-events-auto">
              <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-black italic uppercase tracking-tighter">Pievienot {activeTab === 'fridge' ? 'Ledusskapim' : 'Grozam'}</h3><button onClick={() => setIsAddModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-800"><X size={24}/></button></div>
             <form onSubmit={handleAddCustomItem} className="space-y-4">
                 <div className="space-y-1 relative">
                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-2">Produkta nosaukums</label>
                    <input type="text" required value={newItem.Item} onChange={handleItemNameChange} placeholder="Sāc rakstīt..." className="w-full p-4 rounded-2xl bg-slate-50 border border-slate-100 font-bold italic focus:outline-none focus:border-orange-500" />
                 ...
                 <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-2">Departaments</label>
                 ...
                 <button type="submit" className={`w-full mt-4 p-5 rounded-2xl text-white font-black uppercase italic tracking-widest shadow-lg flex justify-center items-center gap-2 active:scale-95 transition-all ${theme.primary}`}><Plus size={20}/> {activeTab === 'fridge' ? 'Pievienot Ledusskapim' : 'Pievienot Grozam'}</button>
              </form>
           </div>
        </div>
      )}
             ...
             <div className="space-y-4">
                {fridgeItems.length > 0 ? fridgeItems.map(i => (
                    <div key={i.id} className="p-5 px-7 bg-white border border-slate-100 rounded-[1.8rem] flex justify-between items-center shadow-sm">
                       <div className="flex items-center gap-4"><div className={`p-2 rounded-xl bg-slate-50 text-slate-400`}><Package size={18}/></div><div><h3 className="font-black text-[14px] uppercase italic text-slate-700">{i.Item}</h3><p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">{i.Department || "Cits"}</p></div></div>
