...
const CONFIG = {
  ...
  // SALABOTS: Atgriežamies pie stabila Netlify tuneļa
  // Tas nodrošina, ka Netlify pareizi nolasa datus caur _redirects failu
  baseUrl: "/api/notion" 
};
...
export default function App() {
  ...
  const masterItemList = useMemo(() => {
    ...
    recipeIngredients.forEach(ri => {
        ...
        if (name) {
            itemsMap.set(name.toLowerCase(), { name: name, unit: ri.Unit || "pcs", dept: "Other" });
        }
    });
    inventory.forEach(inv => {
        ...
        if (name) {
            ...
            if (existing) {
                itemsMap.set(key, { ...existing, dept: inv.Department || existing.dept });
            } else {
                itemsMap.set(key, { name: name, unit: inv.Unit || "pcs", dept: inv.Department || "Other" });
            }
        }
    });
  ...
  // --- COOKING LOGIC: DEDUCT INGREDIENTS ---
  const handleCookMeal = async () => {
    if (!selectedRecipe || !householdId) return;
    if (!confirm(`Vai tiešām pagatavoji "${selectedRecipe.name}"? Produkti tiks noņemti no ledusskapja.`)) return;
    
    setLoading(true);
    try {
      // 1. Aprēķinām, kas ir nepieciešams šai receptei (ņemot vērā porcijas!)
      const ingredientsToDeduct = recipeIngredients.filter(i => 
          i.forMeals && i.forMeals.includes(selectedRecipe.id)
      );

      const batchPromises = ingredientsToDeduct.map(async (ri) => {
          const normName = (ri.Item || "").trim().toLowerCase();
          const amountToDeduct = ri.BaseAmount * portions; // SVARĪGI: Porciju reizinātājs

          // 2. Meklējam produktu ledusskapī
          ...
          if (fireItem) {
              ...
              const currentStock = fireItem.inStock || 0;
              // Atņemam no krājumiem. Ja aiziet mīnusos -> labi, tātad bija iztrūkums, kas jāpērk.
              const newStock = currentStock - amountToDeduct;
              
              await updateDoc(itemRef, { inStock: newStock });
          } else {
              // Ja produkta vispār nav sarakstā, izveidojam to ar negatīvu atlikumu
              // Tas nozīmē: "Mēs izlietojām X, tātad mums trūkst X"
              const invCol = collection(db, "households", householdId, "inventory");
              await addDoc(invCol, { 
                  Item: ri.Item, 
                  Unit: ri.Unit || "pcs", 
                  Department: "Other", 
                  inStock: -amountToDeduct 
              });
          }
      });

      await Promise.all(batchPromises);
      alert("Labu apetīti! 🍽️ Produkti ir noņemti no ledusskapja.");
      setSelectedRecipe(null); // Atgriežamies uz sākumu
    } catch (e) {
      console.error(e);
      setErrorMsg("Kļūda reģistrējot gatavošanu.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // --- SASTĀVDAĻU APRĒĶINS VIENAI RECEPTEI ---
  const selectedRecipeIngredients = useMemo(() => {
    ...
    return recipeIngredients
        ...
        .map(ri => ({
            ...ri,
            totalAmount: ri.BaseAmount * portions
        }));
